/* tslint:disable */
/* eslint-disable */
/**
 * SUT SA Example API
 * This is a sample server for SUT SE 2563
 *
 * The version of the OpenAPI document: 1.0
 * Contact: support@swagger.io
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
import {
    EntCourseclass,
    EntCourseclassFromJSON,
    EntCourseclassFromJSONTyped,
    EntCourseclassToJSON,
} from './';

/**
 * 
 * @export
 * @interface EntClasstimeEdges
 */
export interface EntClasstimeEdges {
    /**
     * Courseclasses holds the value of the courseclasses edge.
     * @type {Array<EntCourseclass>}
     * @memberof EntClasstimeEdges
     */
    courseclasses?: Array<EntCourseclass>;
}

export function EntClasstimeEdgesFromJSON(json: any): EntClasstimeEdges {
    return EntClasstimeEdgesFromJSONTyped(json, false);
}

export function EntClasstimeEdgesFromJSONTyped(json: any, ignoreDiscriminator: boolean): EntClasstimeEdges {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'courseclasses': !exists(json, 'courseclasses') ? undefined : ((json['courseclasses'] as Array<any>).map(EntCourseclassFromJSON)),
    };
}

export function EntClasstimeEdgesToJSON(value?: EntClasstimeEdges | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'courseclasses': value.courseclasses === undefined ? undefined : ((value.courseclasses as Array<any>).map(EntCourseclassToJSON)),
    };
}


