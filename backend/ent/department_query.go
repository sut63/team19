...
func (dq *DepartmentQuery) QueryInstructorinfos() *InstructorInfoQuery {
	query := &InstructorInfoQuery{config: dq.config}
...
func (dq *DepartmentQuery) QueryDepartment() *CourseQuery {
...
func (dq *DepartmentQuery) WithInstructorinfos(opts ...func(*InstructorInfoQuery)) *DepartmentQuery {
	query := &InstructorInfoQuery{config: dq.config}
	for _, opt := range opts {
		opt(query)
	}
	dq.withInstructorinfos = query
	return dq
}
...
func (dq *DepartmentQuery) WithDepartment(opts ...func(*CourseQuery)) *DepartmentQuery {
	query := &CourseQuery{config: dq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (dq *DepartmentQuery) sqlAll(ctx context.Context) ([]*Department, error) {
	...
	if query := dq.withInstructorinfos; query != nil {
		...
		query.Where(predicate.InstructorInfo(func(s *sql.Selector) {
			s.Where(sql.InValues(department.InstructorinfosColumn, fks...))
		}))
