...
func (cq *CourseQuery) QueryDepartmentID() *DepartmentQuery {
	query := &DepartmentQuery{config: cq.config}
...
// QueryDegreeID chains the current query on the Degree_id edge.
func (cq *CourseQuery) QueryDegreeID() *DegreeQuery {
...
func (cq *CourseQuery) QuerySubjectID() *SubjectQuery {
...
func (cq *CourseQuery) WithDepartmentID(opts ...func(*DepartmentQuery)) *CourseQuery {
	...
	cq.withDepartmentID = query
	return cq
}

//  WithDegreeID tells the query-builder to eager-loads the nodes that are connected to
// the "Degree_id" edge. The optional arguments used to configure the query builder of the edge.
func (cq *CourseQuery) WithDegreeID(opts ...func(*DegreeQuery)) *CourseQuery {
	query := &DegreeQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withDegreeID = query
	return cq
}

//  WithSubjectID tells the query-builder to eager-loads the nodes that are connected to
// the "Subject_id" edge. The optional arguments used to configure the query builder of the edge.
func (cq *CourseQuery) WithSubjectID(opts ...func(*SubjectQuery)) *CourseQuery {
	query := &SubjectQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withSubjectID = query
	return cq
}
...
func (cq *CourseQuery) sqlAll(ctx context.Context) ([]*Course, error) {
	...
	if query := cq.withDepartmentID; query != nil {
		...
		for _, n := range neighbors {
			...
			for i := range nodes {
				nodes[i].Edges.DepartmentID = n
			}
		}
	}

	if query := cq.withDegreeID; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Course)
		for i := range nodes {
		...
		for _, n := range neighbors {
			...
			for i := range nodes {
				nodes[i].Edges.DegreeID = n
			}
		}
	}

	if query := cq.withSubjectID; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Course)
		for i := range nodes {
