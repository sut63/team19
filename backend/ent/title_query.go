...
func (tq *TitleQuery) QueryInstructorinfos() *InstructorInfoQuery {
	query := &InstructorInfoQuery{config: tq.config}
...
func (tq *TitleQuery) WithInstructorinfos(opts ...func(*InstructorInfoQuery)) *TitleQuery {
	query := &InstructorInfoQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withInstructorinfos = query
	return tq
}
...
func (tq *TitleQuery) sqlAll(ctx context.Context) ([]*Title, error) {
	...
	if query := tq.withInstructorinfos; query != nil {
		...
		query.Where(predicate.InstructorInfo(func(s *sql.Selector) {
			s.Where(sql.InValues(title.InstructorinfosColumn, fks...))
		}))
