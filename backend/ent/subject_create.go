...
func (sc *SubjectCreate) AddSubjectsOfferedIDs(ids ...int) *SubjectCreate {
	sc.mutation.AddSubjectsOfferedIDs(ids...)
	return sc
}

// AddSubjectsOffered adds the SubjectsOffered edges to SubjectsOffered.
func (sc *SubjectCreate) AddSubjectsOffered(s ...*SubjectsOffered) *SubjectCreate {
	ids := make([]int, len(s))
	for i := range s {
		ids[i] = s[i].ID
	}
	return sc.AddSubjectsOfferedIDs(ids...)
}

// AddCourseclassIDs adds the courseclasses edge to Courseclass by ids.
func (sc *SubjectCreate) AddCourseclassIDs(ids ...int) *SubjectCreate {
	sc.mutation.AddCourseclassIDs(ids...)
	return sc
}

// AddCourseclasses adds the courseclasses edges to Courseclass.
func (sc *SubjectCreate) AddCourseclasses(c ...*Courseclass) *SubjectCreate {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return sc.AddCourseclassIDs(ids...)
}
...
func (sc *SubjectCreate) createSpec() (*Subject, *sqlgraph.CreateSpec) {
	...
	if nodes := sc.mutation.SubjectsOfferedIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
	...
	if nodes := sc.mutation.CourseclassesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
