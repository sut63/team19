...
func (tq *TermQuery) QuerySubjectsOffered() *SubjectsOfferedQuery {
	query := &SubjectsOfferedQuery{config: tq.config}
...
func (tq *TermQuery) WithSubjectsOffered(opts ...func(*SubjectsOfferedQuery)) *TermQuery {
	query := &SubjectsOfferedQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withSubjectsOffered = query
	return tq
}
...
func (tq *TermQuery) sqlAll(ctx context.Context) ([]*Term, error) {
	...
	if query := tq.withSubjectsOffered; query != nil {
		...
		query.Where(predicate.SubjectsOffered(func(s *sql.Selector) {
			s.Where(sql.InValues(term.SubjectsOfferedColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.term_id
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "term_id" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
