...
type SubjectQuery struct {
	...
	// eager-loading edges.
	withSubject         *CourseQuery
	withSubjectsOffered *SubjectsOfferedQuery
...
func (sq *SubjectQuery) QuerySubject() *CourseQuery {
	query := &CourseQuery{config: sq.config}
...
// QuerySubjectsOffered chains the current query on the SubjectsOffered edge.
func (sq *SubjectQuery) QuerySubjectsOffered() *SubjectsOfferedQuery {
	query := &SubjectsOfferedQuery{config: sq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *SubjectQuery) QueryCourseclasses() *CourseclassQuery {
	query := &CourseclassQuery{config: sq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *SubjectQuery) WithSubject(opts ...func(*CourseQuery)) *SubjectQuery {
	query := &CourseQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withSubject = query
	return sq
}

//  WithSubjectsOffered tells the query-builder to eager-loads the nodes that are connected to
// the "SubjectsOffered" edge. The optional arguments used to configure the query builder of the edge.
func (sq *SubjectQuery) WithSubjectsOffered(opts ...func(*SubjectsOfferedQuery)) *SubjectQuery {
	query := &SubjectsOfferedQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withSubjectsOffered = query
	return sq
}
...
func (sq *SubjectQuery) WithCourseclasses(opts ...func(*CourseclassQuery)) *SubjectQuery {
	query := &CourseclassQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withCourseclasses = query
	return sq
}
...
func (sq *SubjectQuery) sqlAll(ctx context.Context) ([]*Subject, error) {
	var (
		...
		loadedTypes = [3]bool{
			sq.withSubject != nil,
			sq.withSubjectsOffered != nil,
			sq.withCourseclasses != nil,
		}
	)
	...
	if query := sq.withSubject; query != nil {
		...
		query.Where(predicate.Course(func(s *sql.Selector) {
			s.Where(sql.InValues(subject.SubjectColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.Subject_id
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "Subject_id" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "Subject_id" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Subject = append(node.Edges.Subject, n)
		}
	}

	if query := sq.withSubjectsOffered; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Subject)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.withFKs = true
		query.Where(predicate.SubjectsOffered(func(s *sql.Selector) {
			s.Where(sql.InValues(subject.SubjectsOfferedColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node.Edges.SubjectsOffered = append(node.Edges.SubjectsOffered, n)
		}
	}

	if query := sq.withCourseclasses; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Subject)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Courseclass(func(s *sql.Selector) {
			s.Where(sql.InValues(subject.CourseclassesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
